...
add_action( 'delete_category', 'starter_theme_category_transient_flusher' );
 ...
 */
function starter_theme_get_svg( $args = array() ) {

	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return esc_html__( 'Please define default parameters in the form of an array.', 'starter-theme' );
	}

	// YUNO define an icon?
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_html__( 'Please define an SVG icon filename.', 'starter-theme' );
	}
 ...
 */
function starter_theme_do_svg( $args = array() ) {
	echo starter_theme_get_svg( $args );
}
 ...
 */
function starter_theme_get_the_title( $args = array() ) {
 ...
 * @return string        The shortened excerpt.
 */
function starter_theme_get_the_excerpt( $args = array() ) {
 ...
 */
function starter_theme_do_post_image( $size = 'thumbnail' ) {
 ...
 */
function starter_theme_get_post_image_uri( $size = 'thumbnail' ) {
 ...
 */
function starter_theme_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 */
function starter_theme_do_copyright_text() {

	// Grab our customizer settings.
	$copyright_text = get_theme_mod( 'starter_theme_copyright_text' );
 ...
 * @return string
 */
function starter_theme_get_social_share() {
	...
	<div class="social-share">
		<h5 class="social-share-title"><?php esc_html_e( 'Share This', 'starter-theme' ); ?></h5>
		<ul class="social-icons menu menu-horizontal">
			<li class="social-icon">
				<a href="<?php echo esc_url( $twitter_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php starter_theme_do_svg( array( 'icon' => 'twitter-square', 'title' => 'Twitter', 'desc' => __( 'Share on Twitter', 'starter-theme' ) ) ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Twitter', 'starter-theme' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $facebook_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php starter_theme_do_svg( array( 'icon' => 'facebook-square', 'title' => 'Facebook', 'desc' => __( 'Share on Facebook', 'starter-theme' ) ) ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Facebook', 'starter-theme' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $linkedin_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=475, height=505' ); return false;">
					<?php starter_theme_do_svg( array( 'icon' => 'linkedin-square', 'title' => 'LinkedIn', 'desc' => __( 'Share on LinkedIn', 'starter-theme' ) ) ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on LinkedIn', 'starter-theme' ); ?></span>
				</a>
			</li>
		</ul>
	</div><!-- .social-share -->
 ...
 */
function starter_theme_do_mobile_navigation_menu() {

	// Figure out which menu we're pulling.
	$mobile_menu = has_nav_menu( 'mobile' ) ? 'mobile' : 'primary';
?>
	<nav id="mobile-menu" class="mobile-nav-menu">
		<button class="close-mobile-menu"><span class="screen-reader-text"><?php _e( 'Close menu', 'starter-theme' ); ?></span><?php starter_theme_do_svg( array( 'icon' => 'close' ) ); ?></button>
		<?php
			wp_nav_menu( array(
